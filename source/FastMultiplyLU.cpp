...
void FastRHS::transform_ucoe_alpt_to_rhs(const std::vector<const VecMultiD<double>*> & mat_1D, const std::vector<std::string> & operator_type, const int dim_interp, const double coefficient, const int vec_index)
{
    ...
    for (size_t sum_num = 0; sum_num < dim_order_transform.size(); sum_num++)
    {
        transform_multiD_partial_sum_ucoe_alpt(mat_1D, dim_order_transform[sum_num], LU_order_transform[sum_num], operator_type, dim_interp, vec_index, coefficient);
    }
}
		...
		const std::vector<std::string> & LU_order_transform, const std::vector<std::string> & operator_type, const int dim_interp, const int vec_index, const double coefficient)
{
    ...
    for (size_t d = 0; d < dim; d++)
    {
        transform_1D_from_ucoe_alpt_to_rhs(*(mat_1D_array[dim_order_transform[d]]), LU_order_transform[d], operator_type[dim_order_transform[d]], series_vec[d], series_vec[d+1], dim_order_transform[d], is_first_step[d], is_last_step[d], dim_interp, vec_index, coefficient);
    }
}
        ...
        const bool is_first_step, const bool is_last_step, const int dim_interp, const int vec_index, const double coefficient)
{
    ...
    // if first step, then copy value in ucoe_alpt to ucoe_trans_from
    // else, copy value ucoe_trans_to to ucoe_trans_from
    if (is_first_step) { copy_ucoe_alpt_to_transfrom(vec_index); }
    else { copy_transto_to_transfrom(vec_index); }
...
void FastRHS::rhs_2D_coarse_grid(const VecMultiD<double> & mat_x, const VecMultiD<double> & mat_y, const std::string operator_type_x, const std::string operator_type_y, const int dim_interp, const int mesh_nmax,const double coefficient)
{
    assert(dgsolution_ptr->DIM==2);

    rhs_2D_sum_1st_coarse_grid(mat_x, mat_y, operator_type_x, operator_type_y, dim_interp, mesh_nmax, coefficient);

    rhs_2D_sum_2nd_coarse_grid(mat_x, mat_y, operator_type_x, operator_type_y, dim_interp, mesh_nmax, coefficient);
}
...
void FastRHS::rhs_2D_sum_1st_coarse_grid(const VecMultiD<double> & mat_x, const VecMultiD<double> & mat_y, const std::string operator_type_x, const std::string operator_type_y, const int dim_interp, const int mesh_nmax, const double coefficient)
{
    // transform from interpolation basis to alpert basis
    const int pmax_trans_from = Element::PMAX_intp;
    const int pmax_trans_to = Element::PMAX_alpt;
    ...
    // do transformation in 1D in x direction
    transform_1D_coarse_grid(mat_x, "L", operator_type_x, 0, pmax_trans_from, pmax_trans_to, mesh_nmax, coefficient);
    ...
    // do transformation in 1D in y direction
    transform_1D_coarse_grid(mat_y, "full", operator_type_y, 1, pmax_trans_from, pmax_trans_to, mesh_nmax);
    
    // add value in ucoe_trans_to to rhs
    add_transto_to_rhs();
}

void FastRHS::rhs_2D_sum_2nd_coarse_grid(const VecMultiD<double> & mat_x, const VecMultiD<double> & mat_y, const std::string operator_type_x, const std::string operator_type_y, const int dim_interp, const int mesh_nmax, const double coefficient)
{
    // transform from interpolation basis to alpert basis
    const int pmax_trans_from = Element::PMAX_intp;
    const int pmax_trans_to = Element::PMAX_alpt;    
    ...
    // do transformation in 1D in y direction
    transform_1D_coarse_grid(mat_y, "full", operator_type_y, 1, pmax_trans_from, pmax_trans_to, mesh_nmax, coefficient);
    ...
    // do transformation in 1D in x direction
    transform_1D_coarse_grid(mat_x, "U", operator_type_x, 0, pmax_trans_from, pmax_trans_to, mesh_nmax);
...
void FastMultiplyLU::transform_1D_coarse_grid(const VecMultiD<double> & mat_1D, const std::string LU, const std::string operator_type, const int dim_transform, const int pmax_trans_from, const int pmax_trans_to, const int mesh_nmax, const double coefficient, const int vec_index_trans_from, const int vec_index_trans_to)
{   
    const std::vector<std::vector<int>> & index_set_trans_to = (dgsolution_ptr->dg.begin())->second.ucoe_trans_to[vec_index_trans_to].get_index_iterator();
    const std::vector<std::vector<int>> & index_set_trans_from = (dgsolution_ptr->dg.begin())->second.ucoe_trans_from[vec_index_trans_from].get_index_iterator();
	...
	{		
		for (auto & iter : dgsolution_ptr->dg)
		{
			// skip some elements using coarse grid
			int test_sum_level = std::accumulate(iter.second.level.begin(), iter.second.level.end(), 0);
...

			{
				...
				for (auto const & index_trans_to : index_set_trans_to)
				{
					...
					{
						// skip some elements using coarse grid
						int solu_sum_level = std::accumulate(ptr_elem->level.begin(), ptr_elem->level.end(), 0);
...
void FastRHS::copy_ucoe_alpt_to_transfrom(const int vec_index)
{
	for (auto & iter : dgsolution_ptr->dg)
	{
...
void FastInterpolation::transform_ucoealpt_to_upintp_coarse_grid(const std::vector<const VecMultiD<double>*> & alpt_basis_pt_1D, const int mesh_nmax)
{
	const int dim = dgsolution_ptr->DIM;
	...
	for (int sum_num = 0; sum_num < dim_order_transform.size(); sum_num++)
	{
		transform_multiD_partial_sum_coarse_grid(alpt_basis_pt_1D, dim_order_transform[sum_num], LU_order_transform[sum_num], mesh_nmax);
	}
}
...
void FastInterpolation::transform_ucoealpt_to_upintp_coarse_grid(const VecMultiD<double> & alpt_basis_pt_1D, const int mesh_nmax)
{
	const int dim = dgsolution_ptr->DIM;
	const std::vector<const VecMultiD<double>*> alpt_basis_pt_1D_vec(dim, &alpt_basis_pt_1D);

	transform_ucoealpt_to_upintp_coarse_grid(alpt_basis_pt_1D_vec, mesh_nmax);
}
...
void FastInterpolation::transform_1D_from_ucoealpt_to_upintp_coarse_grid(const VecMultiD<double> & mat_1D, const std::string LU, const std::vector<int> & size_trans_from, const std::vector<int> & size_trans_to, const int dim, const bool is_first_step, const bool is_last_step, const int mesh_nmax)
{
	// transform from alpert basis to interpolation basis
	const int pmax_trans_from = Element::PMAX_alpt;
	const int pmax_trans_to = Element::PMAX_intp;

	{
		for (int vec_index = 0; vec_index < Element::VEC_NUM; vec_index++)
		{
			...
			// do transformation in 1D
			transform_1D_coarse_grid(mat_1D, LU, "vol", dim, pmax_trans_from, pmax_trans_to, mesh_nmax, 1.0, vec_index, vec_index);
...
void FastInterpolation::transform_multiD_partial_sum_coarse_grid(const std::vector<const VecMultiD<double>*> & mat_1D_array, const std::vector<int> & dim_order_transform, const std::vector<std::string> & LU_order_transform, const int mesh_nmax)
{
	// transform from alpert basis to interpolation basis
	const int pmax_trans_from = Element::PMAX_alpt;
	const int pmax_trans_to = Element::PMAX_intp;
	...
	for (size_t d = 0; d < dim; d++)
	{
		transform_1D_from_ucoealpt_to_upintp_coarse_grid(*(mat_1D_array[dim_order_transform[d]]), LU_order_transform[d], series_vec[d], series_vec[d + 1], dim_order_transform[d], is_first_step[d], is_last_step[d], mesh_nmax);
	}
}
...
void HyperbolicDiffFluxLagrRHS::rhs_vol_scalar_coarse_grid(const int mesh_nmax)
{
	// volume integral in x direction
	rhs_2D_coarse_grid(oper_matx_lagr_ptr->u_vx, oper_matx_lagr_ptr->u_v, "vol", "vol", 0, mesh_nmax);

	// volume integral in y direction
	rhs_2D_coarse_grid(oper_matx_lagr_ptr->u_v, oper_matx_lagr_ptr->u_vx, "vol", "vol", 1, mesh_nmax);
}
...
void HyperbolicDiffFluxLagrRHS::rhs_flx_intp_scalar_coarse_grid(const int mesh_nmax)
{
	// flux integral in x direction
	rhs_2D_coarse_grid(oper_matx_lagr_ptr->ulft_vjp + oper_matx_lagr_ptr->urgt_vjp, oper_matx_lagr_ptr->u_v, "flx", "vol", 0, mesh_nmax, 0.5);

	// flux integral in y direction
	rhs_2D_coarse_grid(oper_matx_lagr_ptr->u_v, oper_matx_lagr_ptr->ulft_vjp + oper_matx_lagr_ptr->urgt_vjp, "vol", "flx", 1, mesh_nmax, 0.5);
}
...
void FastLagrIntp::eval_up_Lagr_coarse_grid(const int mesh_nmax)
{
	transform_ucoealpt_to_upintp_coarse_grid(alpt_basis_Lagr_pt, mesh_nmax);
}
