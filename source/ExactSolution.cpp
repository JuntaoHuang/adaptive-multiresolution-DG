...
double HJCosExact::exact_1d(const double x, const double t, const int dim)
{
	// since solution is periodic, we make it periodic in the range of 0 < x < 1
	double x_period = x;
	double const d2pi = dim * 2 * pi;
	x_period = std::fmod(x, 1.);
	if (x_period < 0) x_period = std::fmod(x_period + 1, 1.);
	...
	// solve f(x0) := x0 - x + sin(2*sin(4*pi*x0)+1)*t = 0 with Newton's iteration
	// x^(n+1) = G(x^n) := x^n - f(x^n)/f'(x^n)
	auto Gx = [&](double x0) { return x0 - (x0 - x_period + sin(dim*sin(d2pi*x0) + 1.)*t) / (1 + d2pi*dim* cos(dim*sin(d2pi*x0) + 1)*cos(d2pi*x0)*t); };
	...
	// exact solution to HJ equation
	double u = dim*sin(d2pi*x0);
	return -cos(d2pi*x0) / (2.*pi) + (cos(u + 1) + sin(u + 1)*u)*t;
}
