...
double HamiltonianR(std::vector<double> u, const double h = 1.);
...
int main(int argc, char *argv[])
{
	...
	LagrBasis::PMAX = AlptBasis::PMAX + 2;
	...
	auto star = [](std::vector<double> x, double t)
	{
		for (auto & u : x)
			u = std::min(std::max(u - t, 0.), u + t);
		return x;
	...
	auto LFHamiltonian = [=](std::vector<double> u, int i, int d, std::vector<double> x = std::vector<double>())
	{
		...
		return -HamiltonianR(ave, dx) + jump;
	...
	auto final_func1 = [&](const std::vector<double> & x, int i = 0)->double
	{
		double r0 = 1. / 8.;
		...
		for (auto & u : y)
			u = u - 0.5;
		//y[1] = -y[0] - y[1];
		double temp = l2norm(star({y}, final_time));
		return 0.5 / r0 * (temp*temp - r0 * r0);
	};
...
//}
//
double HamiltonianR(std::vector<double> u, const double h)
{
	auto abs_r = [=](double x) { return abs(x) >= h ? abs(x) : 1. / (2.* h) * x * x + 0.5* h; };

	//u[0] = u[0] - u[1];

	double tmp = 0;
	for (const auto s : u) tmp += abs_r(s);
