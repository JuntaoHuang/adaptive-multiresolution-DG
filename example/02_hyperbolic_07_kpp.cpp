...
// -NM 6 -N0 6 -r 1e-3 -c 1e-4 -ssp -v 3 -cfl 0.02 -nu 0.5 -tf 0.2
// -NM 7 -N0 7 -r 1e-4 -c 1e-5 -ssp -v 3 -cfl 0.01 -nu 1.0 -tf 0.2
int main(int argc, char *argv[])
{
	...
	// initial condition: 
	// u0 = 3.5*pi, if (x-1/2)^2 + (y-1/2)^2 <= 1/16,
	// u0 = 0.25*pi, otherwise.
	// since this function is non-separable and discontinuous
	// we first interpolate this function with Lagrange/Hermite interpolation basis function
	// and then transfer coefficient of Lagrange/Hermite basis into coefficient of Alpert basis
	auto init_func = [&](std::vector<double> x, int i, std::vector<int> d)->double 
		{
			if (d[0]==0 && d[1]==0)
			{
				if ( pow(x[0]-0.5, 2.) + pow(x[1]-0.5, 2.) <= 1./16.) { return 3.5 * Const::PI; }
				else { return 0.25 * Const::PI; }
			}
			else
			{
				return 0.;
	...
	HermInterpolation interp_herm_init(dg_solu);
	interp_herm_init.init_from_Herm_to_alpt(init_func, oper_matx_herm);
	dg_solu.coarsen();

	// physical flux
	auto func_flux = [&](std::vector<double> u, int i, int d)->double 
		{
			if (d==0) { return FluxFunction::sin_flux(u[0]); }
			else { return FluxFunction::cos_flux(u[0]); }
		};
	
	// first derivative of physical flux
	auto func_flux_d1 = [&](std::vector<double> u, int i, int d, int i1)->double
		{
			if (d==0) { return FluxFunction::sin_flux_1st_derivative(u[0]); }
			else { return FluxFunction::cos_flux_1st_derivative(u[0]); }
		};

	// second derivative of physical flux
	auto func_flux_d2 = [&](std::vector<double> u, int i, int d, int i1, int i2)->double
		{
			if (d==0) { return FluxFunction::sin_flux_2nd_derivative(u[0]); }
			else { return FluxFunction::cos_flux_2nd_derivative(u[0]); }
		};

	// output
	IO inout(dg_solu);

	// numerical profile and shock support at the initial time
	inout.output_num("profile2D_init.txt");
	inout.output_shock_support("shock_init.txt");
	...
	HyperbolicDiffFluxHermRHS fast_rhs_herm(dg_solu, oper_matx_herm);
	...
	// KPP equation have different flux in x and y direction
	std::vector< std::vector<bool> > is_intp_herm;
	is_intp_herm.push_back(std::vector<bool>(DIM, true));
		...
		{	
			...
			else if (viscosity_option==3)
			{
				...
				{			
					...
					interp_lagr.support_gradu_Lagr_fast(dg_solu.viscosity_element, -artificial_viscosity_nu0*h, is_intp_lagr, fast_lagr_intp);
	...
	// write numerical solution and shock support at final time into file
	inout.output_num("profile2D_final.txt");
	inout.output_shock_support("shock_final.txt");
